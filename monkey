...
###################################################################################################################################################################################################
if [ $# != 1 ];then
	ERROR "使用不正确，例如 (monkey MR7063HC8W1)"
	exit 1
fi

if [ ! -e alps ];then
	ERROR "执行路径不正确，请返回ALPS上级目录执行!!"
	exit 1
fi
###################################################################################################################################################################################################
#全局变量设定
...
if [ ! -z "$CUSTOMBUILDVERSION" ];then
	OK ">>>>>修改自定义编译版本 = $CUSTOMBUILDVERSION  "
...
if [ ! -z "$HOMEPAGE" ];then
	OK ">>>>>修改浏览器主页 = `expr substr $HOMEPAGE 10 20` "
